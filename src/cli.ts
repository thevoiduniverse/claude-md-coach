...
import type { ClaudeMdFile, PerFileFinding, Session } from "./types.js";
...
async function runScan(workspaceRoot: string, quiet: boolean): Promise<void> {
  if (!quiet) process.stderr.write(`Scanning ${workspaceRoot}…\n`);
  ...
  if (!quiet) process.stderr.write(`Found ${discovered.length} CLAUDE.md file(s). Parsing…\n`);

  const files: ClaudeMdFile[] = discovered.map(d =>
    parseClaudeMd(d.path, d.scope, d.projectName),
  );

  if (!quiet) process.stderr.write("Reading session history…\n");
  ...
  if (!quiet) process.stderr.write(`Analyzed ${sessions.length} sessions.\n`);

  const perFile: PerFileFinding[] = [];
  for (const file of files) {
    ...
    for (const t of analysis.perRule) triggerCounts[t.ruleId] = t.triggerCount;
    perFile.push({
      filePath: file.path,
