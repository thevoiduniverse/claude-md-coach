...
export async function discover(workspaceRoot: string): Promise<DiscoveredFile[]> {
...
async function walk(dir: string, root: string, out: DiscoveredFile[]): Promise<void> {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (!CLAUDE_MD_NAMES.has(entry.name)) continue;
