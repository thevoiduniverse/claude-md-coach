...
export function savePending(summary: PendingSummary): void {
...
export function loadPending(): PendingSummary | null {
...
export function formatPendingNudge(s: PendingSummary): string {
  const parts: string[] = [];
  if (s.totalDead > 0) parts.push(`${s.totalDead} rule${s.totalDead === 1 ? "" : "s"} with no trigger`);
  if (s.totalContradictions > 0) parts.push(`${s.totalContradictions} contradiction${s.totalContradictions === 1 ? "" : "s"}`);
  if (s.totalDrafts > 0) parts.push(`${s.totalDrafts} missing-rule suggestion${s.totalDrafts === 1 ? "" : "s"}`);
...
export function runPending(): void {
  ...
  else process.stdout.write("No pending insights.\n");
}
