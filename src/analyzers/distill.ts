import type { ClaudeMdFile, Section, Session } from "../types.js";
...
export interface ProgressiveIteration {
  iteration: number;
  beforeLines: number;
  afterLines: number;
  similarity: number;
...
export interface ProgressiveResult {
  ...
  finalLines: number;
  reductionPct: number;
  iterations: ProgressiveIteration[];
  stoppedReason: "max_iterations" | "verification_failed" | "no_reduction" | "compression_failed";
}

export function pickDistillCandidates(file: ClaudeMdFile): DistillCandidate[] {
...
export async function verifyReplay(
  ...
  for (const s of picked) {
    ...
    const responseA = await runner.run(
      `User's first prompt in a coding session: "${userPrompt.slice(0, 800)}"\n\nGive your initial response plan (2-4 sentences, no code yet). Focus on what you'd do first.`,
      { model: "haiku", systemPrompt: candidate.originalText },
    );
    const responseB = await runner.run(
      `User's first prompt in a coding session: "${userPrompt.slice(0, 800)}"\n\nGive your initial response plan (2-4 sentences, no code yet). Focus on what you'd do first.`,
      { model: "haiku", systemPrompt: compressedSection },
    );

    if (responseA.error || responseB.error) continue;
...
async function scoreSimilarity(
  ...
  b: string,
  runner: ClaudeRunner,
): Promise<{ similarity: number; notes: string }> {
...
${b.slice(0, 1500)}`;
  ...
  const r = await runner.runJson<{ similarity: number; notes: string }>(prompt, { model: "haiku" });
  if (!r || typeof r.similarity !== "number") return { similarity: 0, notes: "scoring failed" };
  return { similarity: Math.max(0, Math.min(100, Math.round(r.similarity))), notes: r.notes ?? "" };
}

function pickReplaySessions(sessions: Session[], n: number): Session[] {
  ...
  while (picked.length < n && picked.length < sorted.length) picked.push(sorted[picked.length]);
  return picked;
}

function firstSubstantiveUserPrompt(s: Session): string | null {
  for (const e of s.events) {
    if (e.type === "user_message" && e.content.trim().length > 30) return e.content;
  }
  return null;
}

export async function progressiveDistill(
  candidate: DistillCandidate,
  sessions: Session[],
  runner: ClaudeRunner,
  opts: { maxIterations?: number; minSimilarity?: number; sampleSize?: number } = {},
  ...
  const iterations: ProgressiveIteration[] = [];
  let stoppedReason: ProgressiveResult["stoppedReason"] = "max_iterations";

  for (let i = 0; i < maxIterations; i++) {
    const innerCandidate: DistillCandidate = {
    ...
    const driftCheck: DistillCandidate = {
      section: candidate.section,
