import type { Rule, Session } from "../types.js";
import { ClaudeRunner } from "../llm/runner.js";

export interface SemanticVerdict {
  ruleId: string;
  verdict: "followed" | "violated" | "irrelevant" | "unclear";
  reason: string;
}

export async function classifyRuleAgainstSessions(
  rules: Rule[],
  sessions: Session[],
  runner: ClaudeRunner,
): Promise<SemanticVerdict[]> {
  if (rules.length === 0 || sessions.length === 0) return [];
...
Output a JSON array with one object per rule: { "ruleId": "R#", "verdict": "...", "reason": "one short sentence" }. JSON only.`;
  ...
  const result = await runner.runJson<SemanticVerdict[]>(prompt, { model: "haiku" });
  if (!result || !Array.isArray(result)) return [];
  return result.filter(v => v && typeof v.ruleId === "string");
}

function pickRepresentative(sessions: Session[], n: number): Session[] {
...
function summarize(s: Session): string {
  ...
  if (userMsgs.length) parts.push("User messages:\n" + userMsgs.join("\n"));

  const files = s.filesTouched.slice(0, 5).join(", ");
  if (files) parts.push(`Files touched: ${files}`);
