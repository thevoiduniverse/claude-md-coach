...
export function extractCorrections(sessions: Session[]): CorrectionSignal[] {
  ...
  for (const s of sessions) {
    for (let i = 0; i < s.events.length; i++) {
      ...
      if (text.length < 3 || text.length > 2000) continue;
      if (!isCorrectionCandidate(text)) continue;
...
function isCorrectionCandidate(text: string): boolean {
...
function findPrecedingTool(events: SessionEvent[], idx: number): SessionEvent | undefined {
  for (let j = idx - 1; j >= Math.max(0, idx - 5); j--) {
    if (events[j].type === "tool_use") return events[j];
  }
  return undefined;
}
...
${numbered}`;
  ...
  const result = await runner.runJson<Array<{
    theme: string;
    indices?: number[];
