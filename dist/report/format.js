...
export function formatReport(input) {
    ...
    lines.push("");
    lines.push(`Found ${BOLD}${input.files.length}${RESET} CLAUDE.md file${input.files.length === 1 ? "" : "s"}, analyzed ${BOLD}${input.sessionsAnalyzed}${RESET} session${input.sessionsAnalyzed === 1 ? "" : "s"}.`);
    lines.push("");
    for (let i = 0; i < input.files.length; i++) {
        ...
        lines.push(`${BOLD}${i + 1}.${RESET} ${shortPath(file.path)}  ${DIM}${scopeLabel}, ${file.lines} lines, ${file.rules.length} rules, ${finding.sessionsInScope} sessions in scope${RESET}`);
