...
import { draftRulesInUserVoice } from "../analyzers/rule-generator.js";
import { detectContradictions } from "../analyzers/contradictions.js";
...
import { analyzeScopeMove } from "../analyzers/scope-move.js";
...
const CYAN = "\x1b[36m";
const RESET = "\x1b[0m";
export async function runFix(workspaceRoot) {
    ...
    try {
        for (let i = 0; i < files.length; i++) {
            ...
            if (deadAnalysis.neverTriggered.length > 0) {
                ...
                const verdicts = await classifyRuleAgainstSessions(deadAnalysis.neverTriggered, scoped, runner);
                const irrelevant = new Set(verdicts.filter(v => v.verdict === "irrelevant").map(v => v.ruleId));
                ...
                if (violated.length > 0) {
                    ...
                    for (const v of violated.slice(0, 5)) {
                        ...
                        if (r)
                            stdout.write(`  ${DIM}${v.ruleId}${RESET} ${r.text.slice(0, 80)} — ${DIM}${v.reason}${RESET}\n`);
                    }
                    stdout.write("\n");
                }
...
async function reviewDeadRules(rl, candidates, file, edits) {
...
async function reviewContradictions(rl, contradictions, file, edits) {
    ...
    for (const c of contradictions) {
        ...
        if (c.unified)
            stdout.write(`  ${CYAN}→ Unified: ${c.unified}${RESET}\n`);
...
async function reviewDrafts(rl, drafts, file, edits) {
...
async function reviewScopeMoves(rl, suggestions, edits) {
...
async function reviewCrossFile(rl, findings, edits) {
