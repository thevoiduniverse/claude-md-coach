...
export class ClaudeRunner {
    ...
    async run(prompt, opts = {}) {
        ...
        const result = await withSpinner(`Calling Claude (${model})`, () => this.invoke(prompt, model, opts.systemPrompt, opts.timeoutMs ?? 90000));
    ...
    async runJson(prompt, opts = {}) {
        const result = await this.run(prompt, opts);
        if (result.error)
            return null;
        return extractJson(result.text);
    }
    keyFor(prompt, model, systemPrompt) {
        const h = createHash("sha256");
        h.update(model + "\n" + (systemPrompt ?? "") + "\n" + prompt);
        return h.digest("hex").slice(0, 24);
    }
    invoke(prompt, model, systemPrompt, timeoutMs) {
...
export function extractJson(text) {
    ...
    for (const c of candidates) {
        const parsed = tryParseJsonBlock(c);
        if (parsed !== null)
            return parsed;
    }
    return null;
